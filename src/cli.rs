...
fn cmd() -> clap::Command {
    clap::command!("ninja")
        ...
        .arg(arg!(-p --package <SPEC>  "Package to build (see `cargo help pkgid`)")
            .num_args(0..=1)
        .action(ArgAction::Append))
        ...
        .arg(arg!(--bin  <NAME>       "Build only the specified binary")
            .num_args(0..=1))
        .arg(arg!(--examples          "Build all examples"))
        .arg(arg!(--example  <NAME>   "Build only the specified example")
            .num_args(0..=1))
        .arg(arg!(--tests             "Build all test targets"))
        .arg(arg!(--test  <NAME>      "Build only the specified test target")
            .num_args(0..=1))
        .arg(arg!(--benches           "Build all bench targets"))
        .arg(arg!(--bench  <NAME>     "Build only the specified bench target")
            .num_args(0..=1))
        ...
        .arg(arg!(--target <TRIPLE>       "Build for the target triple")
            .num_args(0..=1))
        .arg(arg!(--timings <FMTS>        "Timing output formats (unstable) (comma separated): html, json")
